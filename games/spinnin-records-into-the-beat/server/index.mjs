/**
 * Spinnin' Records Into the Beat — dev + production server (Pixi client + JSON spin API).
 */
...
app.get('/health', (_req, res) => {
  res.json({ ok: true, game: 'spinnin-records-into-the-beat', env: IS_PRODUCTION ? 'production' : 'development' });
});
...
server.listen(PORT, () => {
  console.log(`Spinnin' Records Into the Beat (PixiJS) — http://localhost:${PORT}/`);
