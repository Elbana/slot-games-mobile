/**
 * JSON spin API — Spinnin' Records Into the Beat
 */
import { spinFootballGold } from './games/spinnin-records-into-the-beat/spin.mjs';
import { effectiveBet } from './games/spinnin-records-into-the-beat/freespin.mjs';
import { BET_LEVELS, RATE_LIMIT_SPIN_MS, REQUIRE_AUTH } from './config.mjs';
import { loadSession, saveSession, cacheSpinResult, getCachedSpin } from './session-store.mjs';
import { auditSpin } from './audit.mjs';

const GAME = 'spinnin-records-into-the-beat';
...
export function handleV2Spin(req, res) {
  ...
  const result = spinFootballGold(session, bet, { spinId: spinId ?? Date.now() });
